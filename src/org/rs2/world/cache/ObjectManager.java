...
public class ObjectManager {
	...
	public static void appendStateChange(int objectType, int objectX, int objectY,
		int objectHeight, int objectFace, int objectChangeState, int objectVType) {
		stateChanges.add(new StateObject(objectType, objectX, objectY, objectFace, objectHeight,
			objectChangeState, objectVType));
	}

	public static void appendVarObject(int objectType, int objectX, int objectY, int objectHeight,
		int objectFace) {
	...
	public static void removeStateChange(int objectType, int objectX, int objectY, int objectHeight) {
		for (int index = 0; index < stateChanges.size(); index++) {
			...
			if ((so.getX() == objectX && so.getY() == objectY && so.getHeight() == objectHeight)
				&& so.getType() == objectType || so.getStatedObject() == objectType) {
	...
	public static void removeVarObject(int objectType, int objectX, int objectY, int objectHeight) {
		for (int index = 0; index < varObjects.size(); index++) {
			...
			if (vo.getId() == objectType && vo.getX() == objectX && vo.getY() == objectY
				&& vo.getHeight() == objectHeight) {
	...
	private static void loadCachedObjects() {
		try {
			...
			for (int index = 0; index < length; index++) {
				cachedObjects.put(dis.readInt(),
					new CachedObject(dis.readUnsignedShort(), dis.readByte(), dis.readByte()));
			}
			dis.close();
	...
	public static StateObject getStateObject(int objectX, int objectY, int objectHeight,
		int objectId) {
		for (StateObject so : stateChanges) {
			if (so == null) {
				continue;
	...
	public static boolean withinRange(int objectType, int objectX, int objectY, int playerX,
		int playerY, int atHeight) {
		...
		java.awt.Rectangle playerField = new java.awt.Rectangle(objectX - 1, (objectY - 1),
			(sizeX + 2), (sizeY + 2));
		return playerField.contains(playerX, playerY) && !objectField.contains(playerX, playerY);
	}

	public static boolean withinDoorRange(int objectType, int objectX, int objectY, int playerX,
		int playerY, int atHeight) {
		if (objectSizes.size() == 0) {
			loadObjectSizes();
		}
